...
exports.run = async (client, message, args) => {
    if(!message.member.roles.cache.some(role =>["Ranking Permissions", "Ranking Admin"].includes(role.name))){
        return message.channel.send({embed: {
            color: 10038562,
            description: "You need the `Ranking Permissions` or `Ranking Admin` role to run this command.",
            author: {
